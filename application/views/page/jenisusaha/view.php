	...
	if ($("#formTambah").length > 0) {
		$("#formTambah").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					url: "<?php echo base_url('jenisusaha/simpan') ?>",
					type: "POST",
					...
					success: function(response) {
						...
						if (response == true) {
							...
							swalIdDouble('Nama Usaha Sudah digunakan!');
						} else {
							swalInputFailed();
						}
						// setTimeout(function(){
				...
				$.ajax({
					...
					success: function(response) {
						...
						if (response == true) {
							...
							swalIdDouble('Nama jenis usaha Sudah digunakan!');
						} else {
							swalEditFailed();
						}
					}
...
<script type="text/javascript">
	...
	function show_data() {
		$.ajax({
			type: 'POST',
			url: '<?php echo site_url('jenisusaha/tampil') ?>',
			async: true,
			dataType: 'json',
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('jenisusaha/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
