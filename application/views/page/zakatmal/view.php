					...
					<div class="col-xs-12">
						...
						<form class="form-horizontal" role="form" enctype="multipart/form-data" id="formImport">
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<a href="<?php echo base_url() . 'zakatmal/downloadsample' ?>" class="col-sm-3" for="form-field-1"> Download Sample Format</a>
								</div>
							</div>
					</div>
				</div>
					...
					<div class="col-xs-12">
						...
						<form class="form-horizontal" enctype="multipart/form-data" role="form" id="formTambah">
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" id="npwp" readonly name="npwp" placeholder="NPWZ" class="form-control" />
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Jenis Zakat </label>
								<div class="col-sm-6">
									<select class="form-control" name="jeniszakat" id="jeniszakat">
										<option value="">-- Pilih --</option>
										<?php foreach ($myjeniszakat as $value) {
										?>
											<option value=<?= $value['id'] ?>><?= $value['nama'] ?></option>
										<?php }
										?>
					...
					<div class="col-xs-12">
						...
						<form class="form-horizontal" role="form" id="formEdit">
							...
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Jenis Zakat </label>
								<div class="col-sm-6">
									<select class="form-control" name="e_jeniszakat" id="e_jeniszakat">
										<option value="">-- Pilih --</option>
										<?php foreach ($myjeniszakat as $value) {
										?>
											<option value=<?= $value['id'] ?>><?= $value['nama'] ?></option>
...
<script type="text/javascript">
	...
	if ($("#formImport").length > 0) {
		$("#formImport").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('zakatmal/import') ?>",
					data: formdata,
					processData: false,
					contentType: false,
					cache: false,
	...
	if ($("#formTambah").length > 0) {
		$("#formTambah").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('zakatmal/simpan') ?>",
					data: formdata,
					processData: false,
					contentType: false,
					cache: false,
	...
	if ($("#formEdit").length > 0) {
		$("#formEdit").validate({
			...
			submitHandler: function(form) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('zakatmal/update') ?>",
					dataType: "JSON",
	...
	$('#show_data').on('click', '.item_hapus', function() {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('zakatmal/delete') ?>",
					async: true,
					dataType: "JSON",
					data: {
						id: id,
	...
	$('#show_data').on('click', '.item_print', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('zakatmal/laporan_pdf') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('zakatmal/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
	...
	function show_data() {
		$.ajax({
			type: 'POST',
			url: '<?php echo site_url('zakatmal/tampil') ?>',
			async: true,
			dataType: 'json',
			success: function(data) {
				var html = '';
				...
				for (i = 0; i < data.length; i++) {
					html += '<tr>' +
						...
						'</button> &nbsp' +
						'<a target="_blank"  href="<?php echo base_url() . 'zakatmal/laporan_pdf?id=' ?>' + data[i].id +'" class="btn btn-xs btn-success" title="Print" data-id="' + data[i].id + '">' +
