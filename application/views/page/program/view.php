					...
					<div class="col-xs-12">
						...
						<form class="form-horizontal" role="form" enctype="multipart/form-data" id="formImport">
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<a href="<?php echo base_url() . 'assets/master_program.xlsx' ?>" class="col-sm-3" for="form-field-1"> Download Sample Format</a>
								</div>
							</div>
					</div>
				</div>
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" id="nama" name="nama" placeholder="Nama Program" class="form-control" />
								</div>
							</div>
						</div>
					</div>
						...
						<div class="col-xs-12">
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									...
									<input type="text" id="e_nama" name="e_nama" placeholder="Nama Program" class="form-control" />
								</div>
							</div>
						</div>
					</div>
	...
	if ($("#formImport").length > 0) {
		$("#formImport").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('program/import') ?>",
					data: formdata,
					processData: false,
					contentType: false,
					cache: false,
	...
	if ($("#formTambah").length > 0) {
		$("#formTambah").validate({
			...
			messages: {
				...
				id: {
					required: "Kode Jenis Program harus diisi!"
					// ,maxlength: "Your last name maxlength should be 50 characters long."
				},
				nama: {
					required: "Nama Jenis Program harus diisi!"
			...
			submitHandler: function(form) {
				...
				$.ajax({
					url: "<?php echo base_url('program/simpan') ?>",
					type: "POST",
					...
					success: function(response) {
						...
						if (response == true) {
							...
							swalIdDouble('Nama Program Sudah digunakan!');
						} else {
							swalInputFailed();
						}
						// setTimeout(function(){
				...
				$.ajax({
					...
					success: function(response) {
						...
						if (response == true) {
							...
							swalIdDouble('Nama jenis Program Sudah digunakan!');
						} else {
							swalEditFailed();
						}
					}
...
<script type="text/javascript">
	...
	function show_data() {
		$.ajax({
			type: 'POST',
			url: '<?php echo site_url('program/tampil') ?>',
			async: true,
			dataType: 'json',
			success: function(data) {
				var html = '';
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('program/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
