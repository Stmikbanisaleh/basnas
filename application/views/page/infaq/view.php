					...
					<div class="col-xs-12">
						...
						<form class="form-horizontal" role="form" enctype="multipart/form-data" id="formImport">
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<a href="<?php echo base_url() . 'infaq/downloadsample' ?>" class="col-sm-3" for="form-field-1"> Download Sample Format</a>
								</div>
							</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	if ($("#formImport").length > 0) {
		$("#formImport").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('infaq/import') ?>",
					data: formdata,
					processData: false,
					contentType: false,
					cache: false,
	...
	if ($("#formTambah").length > 0) {
		$("#formTambah").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('infaq/simpan') ?>",
					data: formdata,
					processData: false,
					contentType: false,
					cache: false,
	...
	if ($("#formEdit").length > 0) {
		$("#formEdit").validate({
			...
			submitHandler: function(form) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('infaq/update') ?>",
					dataType: "JSON",
	...
	$('#show_data').on('click', '.item_hapus', function() {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('infaq/delete') ?>",
					async: true,
					dataType: "JSON",
					data: {
						id: id,
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('infaq/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
	...
	function show_data() {
		$.ajax({
			type: 'POST',
			url: '<?php echo site_url('infaq/tampil') ?>',
			async: true,
			dataType: 'json',
			success: function(data) {
				var html = '';
