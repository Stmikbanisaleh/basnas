...
class Infaq extends CI_Controller
{
	...
	public function downloadsample()
	{
		...
		$id_zakatfitrah = $this->model_infaq->trxinfaq()->result_array();
		$data = $id_zakatfitrah;
		$no = 1;
		$row = 2;
		if (count($data) > 0) {
			if ($data) {
				...
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D2', '6');
				...
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E3', '3');
	...
	public function laporan_pdf()
	{
		...
		$mydata = $this->model_infaq->laporan_infaq('master_zakat', $id)->result();
		$filename = "Tanda-Terima-Infaq". "-" .$mydata[0]->nama . "-" . date('Y-m-d');
		$data = array(
			'my_data' => $mydata,
			'filename'	=> $filename
		);
		$this->load->view('page/infaq/transaksi_infaq', $data);
